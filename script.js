...
function init() {
  ...
  function createActionButtons(game) {
    ...
    return `
        ${
          game.steamId
            ? `<button class="action-btn steam-btn" onclick="window.open('https://store.steampowered.com/app/${game.steamId}', '_blank')"><i class="fab fa-steam"></i></button>`
            : ""
        }
        ${
          playstationUrl
            ? `<button class="action-btn ps-btn" onclick="window.open('${playstationUrl}', '_blank')"><i class="fab fa-playstation"></i></button>`
            : ""
        }
        ${
          game.xboxStoreId
            ? `<button class="action-btn xbox-btn" onclick="window.open('https://www.xbox.com/en-us/games/store/${game.xboxStoreId}', '_blank')"><i class="fab fa-xbox"></i></button>`
            : ""
        }
        ${
          game.googlePlayStoreId
            ? `<button class="action-btn google-btn" onclick="window.open('https://play.google.com/store/apps/details?id=${game.googlePlayStoreId}', '_blank')"><i class="fab fa-google-play"></i></button>`
            : ""
        }
        ${
          game.videoId
  ...
  function createGameDetails(game) {
    ...
    if (game.releaseDates && Array.isArray(game.releaseDates)) {
      game.releaseDates.forEach((release) => {
        ...
        releaseDatesHtml += `
                    <div class="release-date ${release.type
                      ?.toLowerCase()
                      .replace(" ", "-")}${
          release.platform ? " platformed" : ""
        }">
                        ${
                          release.platform
                            ? `<span class="platform-label platform-${
                                release.platform
                              }">${platformLabel}</span> – ${typeLabel}: ${formatDate(
                                release.date
                              )}`
    ...
    if (game.platforms) {
      game.platforms.forEach((platform) => {
        ...
        if (platformLower.includes("pc"))
          platformTags +=
            '<span class="platform-tag" title="PC"><i class="fab fa-windows"></i></span>';
        if (
          platformLower.includes("playstation") ||
          platformLower.includes("ps")
        )
          platformTags +=
            '<span class="platform-tag" title="PlayStation"><i class="fab fa-playstation"></i></span>';
        if (platformLower.includes("xbox"))
          platformTags +=
            '<span class="platform-tag" title="Xbox"><i class="fab fa-xbox"></i></span>';
        if (
          platformLower.includes("android") ||
          platformLower.includes("mobile")
        )
          platformTags +=
            '<span class="platform-tag" title="Mobile/Android"><i class="fab fa-android"></i></span>';
      });
    }
